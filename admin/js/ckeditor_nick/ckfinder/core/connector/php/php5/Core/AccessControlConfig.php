...
class CKFinder_Connector_Core_AccessControlConfig
{
    ...
    public function getComputedMask($resourceType, $folderPath)
    {
        ...
        for($i = -1; $i < sizeof($_pathParts); $i++) {
            if ($i >= 0) {
                ...
                $_computedMask = $this->mergePatchomputedMask( $_computedMask, $resourceType, $_userRole, $_currentPath . '*/' );
     ...
     * @return int
     */
    private function mergePatchomputedMask( $currentMask, $resourceType, $userRole, $path )
    {
