<?php
	...
	if($errMsg == ""){
		...
		foreach($orNoArr as $key=>$value){
			...
			$data = $or->getSql($sql);
			$Record = "insert into service_record (rcNo,aauNoService,content,time) value('".$data['0']['rcNo']."','".$_SESSION['userdata']['smName']."','使用Edit8狀態由".($or->statusArr[$orOldData[0]["orStatus"]])."變成".($or->statusArr[$orStatusArr[$key]])."','".date('Y-m-d H:i:s',time())."')";
			$or->getSql($Record);
			}
