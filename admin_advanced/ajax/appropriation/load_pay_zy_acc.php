...
if(!isset($errMsg)){
	...
	$outputData = array();
	$formate = explode('-',substr($contentLines[0],10,10));
	if(DateTime::createFromFormat('Ymd', $formate['0'].$formate['1'].$formate['2']) !== FALSE){
		$prArr = array();
		foreach($contentLines as $keyPerson=>$valPerson){
			$rcRealCaseNo = "20".substr($valPerson,72,10);
			...
			if($rcData[0]["rcApproStatus"] == "4"){
				...
				$actualPayTotal = ltrim(substr($valPerson,40,10),'0');
				$payDate = date("Y-m-d",strtotime(substr($valPerson,10,10)));
		...
		if(empty(array_filter($outputData))){
			...
			if(move_uploaded_file($_FILES["cmcSuccessGet"]["tmp_name"], "../../file/payment/zy_acc/".$newFilePath)) {
				$fileName = "file/payment/zy_acc/".$newFilePath;
			}else{
				$fileName = "上傳失敗";
			}
			$purDataInput = array(
