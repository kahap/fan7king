<?php
	...
	foreach($_POST as $key => $value){
		if($value== ""){
			if($key != "memOther" && $key != "memLineId" && $key != "memSubmail"){
				if($_POST['memClass'] == '0'){
					$errg[] = $colum1[$key];
				}				
			}
	...
	$msg = implode(',',$errg);
	if($errg == ""){
		if($memberData['0']['memEmailAuthen'] == 0 && $_POST['memClass'] == '0'){
			/*if($_POST['memAccount'] != "" && $_POST['memClass'] == '0'){
								...
								<tbody>
									...
									<tr>
										<td style="font-weight:bold;background-color:#F5F3F1;">
											...
											<p>
												<a href=https://happyfan7.com/php/member_id.php?pass_number='.$_POST['pass_number'].'&memNo='.$_POST['memNo'].' target="_blank">https://happyfan7.com/php/member_id.php?id='.$_POST['pass_number'].'</a>
											</p>
				...
				$send = $email->SendEmail_smtp($receiverNameAndEmails,$_POST['memName'], "happyfan@happyfan7.com", "樂分期", $title, $content);	
			}*/
			$_POST['memBday'] = $_POST['year']."-".$_POST['month']."-".$_POST['date'];
			$member->update_fornMember_FB($_POST,$_SESSION['user']['memNo']);
			echo "1";
		}else{
