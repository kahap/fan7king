...
//業務人員報表
    ...
    header('Content-Disposition: attachment; filename='.date('Y-m-d',time()).'機車貸款報表.xls');  //設定檔案名稱
	$member = new Member();
	$db = new WADB(SYSTEM_DBHOST, SYSTEM_DBNAME, SYSTEM_DBUSER, SYSTEM_DBPWD);
	$sql = "SELECT a.rcCaseNo,a.rcStatus,a.rcRelateDataNo,a.rcBankTransferAmount,a.`rcDate`,a.rcStatus3Time,a.rcPeriodTotal,a.rcPeriodAmount,b.mcoNo,b.mcoMinMonthlyTotal,d.memName FROM `real_cases` a,`motorbike_cellphone_orders` b,`member` d WHERE a.`rcDate` BETWEEN CONCAT(SUBSTRING(DATE(NOW()),1,8),'01') AND NOW() and a.`rcCaseNo` != '' and a.rcType = '2' and b.mcoNo = a.rcRelateDataNo and b.memNo = d.memNo order by a.rcCaseNo asc";
