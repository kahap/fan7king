<?php
	class Recomm_Bonus_Apply{
		...
		//訂單編號取得單一獎金
		public function getOneRBAByOrNoRBAByMemNo($rbaRecMemNo,$rbamemNo){
			$sql = "select
						*
					from
						`recomm_bonus_apply`
		...
		public function getRBAByMemNo($rbaRecMemNo){
			//$sql = "SELECT * FROM `recomm_bonus_apply` WHERE `rbaRecMemNo` = '".$rbaRecMemNo."' or `rbamemNo` = '".$rbaRecMemNo."'";
			$sql = "SELECT * FROM `recomm_bonus_apply` WHERE `rbaRecMemNo` = '".$rbaRecMemNo."'";
			$data = $this->db->selectRecords($sql);
			return $data;
		}
		...
		function insert($array){
			...
			$sql = "insert into `recomm_bonus_apply`(`orNo`, `rbaRecMemNo` ,`rbaStatus`,`rbamemNo`,
					`rbaDate` )
					values('".$orNo."',
							'".$rbaRecMemNo."',
							'".$rbaStatus."',
