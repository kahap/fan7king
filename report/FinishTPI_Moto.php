...
//業務人員報表
    ...
    header('Content-Disposition: attachment; filename='.date('Y-m-d',time()).'機車分期結清案件.xls');  //設定檔案名稱*/
	$member = new Member();
	...
	//date("d",time())
	$sql = "SELECT a.rcCaseNo,a.memNo, a.rcType, a.rcRelateDataNo, a.rcPeriodAmount, b.tpiSupposeDate, b.tpiPeriodTotal,b.tpiActualDate, c.memName, c.memCell, d.mcoMotorBrand FROM `real_cases` a, tpi b,member c, motorbike_cellphone_orders d where a.rcNo = b.rcNo && a.rcPeriodAmount = b.tpiPeriod && b.tpiActualDate != '' && a.memNo = c.memNo && a.rcRelateDataNo = d.mcoNo && a.rcType = '2' group by a.rcNo";
